 ...
 */
export function isOperationUsed(
  operation: string,
  type: string,
  filePath: string,
): boolean {
  try {
    const operationName = capitalizeFirstLetter(operation);
    const operationType = capitalizeFirstLetter(type);
    const usagePattern = `use${operationType}${operationType}`;
    const documentPattern = `${operationName}Document`;
    const queryPattern = `query ${operationName}`;
    const hasuraAction = `DbService.${operationName}`;
    const content = fs.readFileSync(filePath, 'utf-8');
    return (
      content.includes(usagePattern) ||
      content.includes(documentPattern) ||
      content.includes(queryPattern) ||
      content.includes(hasuraAction)
    );
  } catch (error) {
